...
class Random:
    ...
    # function returns a random integer according to a categorical distribution
    def Categorical(self, ps):

        R = self.rand()
        # for a n-sided die, the possible outputs are 1..N, not 0..N-1
        num = 1

        # check which bin the number is in
        for i in range(len(ps)):
            # due to the way the probability arrays are input, checking the bins is straightforward.
            # the side probabilities aren't guaranteed to be identical, so to find a proper roll, we
            # see if the random number is between some cumulative probabilities 
            # left = sum(0..P(i)) and right = sum(0..P(i+1))
